...
public class MyRestService {
	...
	// ///////////////////////////// [START] FOR MULTIPART FORM DATA... IF
	// REQUIRED ////////////////////////////////

	/*
	 * @RequestMapping(value = "/upload", method=RequestMethod.POST) public
	 * @ResponseBody ResponseModel uploadFile(@RequestParam("uploadedFile")
	 * MultipartFile uploadedFileRef) {
	 * 
	 * Loader.init();
	 * 
	 * FeaturesORB orb = new FeaturesORB(); queryDescriptor = new Mat(); matches
	 * = new MatOfDMatch(); List<SimilarityIndex> similarIndices = new
	 * ArrayList<SimilarityIndex>();
	 * 
	 * try { InputStream inS = uploadedFileRef.getInputStream();
	 * descriptorMatcher =
	 * DescriptorMatcher.create(DescriptorMatcher.BRUTEFORCE_HAMMING);
	 * queryDescriptor =
	 * orb.getORBFeaturesDescriptorMat(Converter.img2Mat(ImageIO.read(inS)));
	 * 
	 * for (int i = 0; i < posters_TrainDescriptors.size(); i++) {
	 * descriptorMatcher.clear(); trainDescriptor =
	 * posters_TrainDescriptors.get(i); descriptorMatcher.match(queryDescriptor,
	 * trainDescriptor, matches);
	 * 
	 * List<DMatch> matchesList = matches.toList();
	 * 
	 * Double max_dist = 0.0; Double min_dist = 100.0;
	 * 
	 * for (int j = 0; j < queryDescriptor.rows(); j++) { Double dist = (double)
	 * matchesList.get(j).distance; if (dist < min_dist) min_dist = dist; if
	 * (dist > max_dist) max_dist = dist; }
	 * 
	 * // -- Draw only "good" matches
	 * 
	 * LinkedList<DMatch> good_matches = new LinkedList<>(); double
	 * goodMatchesSum = 0.0;
	 * 
	 * // good match = distance > 2*min_distance ==> put them in a list for (int
	 * k = 0; k < queryDescriptor.rows(); k++) { if (matchesList.get(k).distance
	 * < Math.max(2 * min_dist,0.02)) {
	 * good_matches.addLast(matchesList.get(k)); goodMatchesSum +=
	 * matchesList.get(k).distance; } }
	 * 
	 * double simIndex = (double) goodMatchesSum/ (double) good_matches.size();
	 * similarIndices.add(new SimilarityIndex(simIndex, posterUrls.get(i),
	 * posterNames.get(i))); }
	 * 
	 * 
	 * /////////sorting///////////// Comparator<SimilarityIndex> indexComparator
	 * = new Comparator<SimilarityIndex>() { public int compare(SimilarityIndex
	 * index1, SimilarityIndex index2) { return
	 * index1.getIndex().compareTo(index2.getIndex()); } };
	 * 
	 ...
	 * ////////////BestOptions adding////////////
	 * 
	 * bestURLS.clear(); bestNames.clear(); IMDBDetials.clear(); for(int i=0;
	 * i<5 ; i++) { bestNames.add(i, similarIndices.get(i).getName());
	 * bestURLS.add(i, similarIndices.get(i).getUrl()); IMDBDetials.add(i,
	 * getImdbData(similarIndices.get(i).getName())); }
	 * 
	 * ////////////////end best option getting///////////
	 * 
	 * } catch (Exception e) { e.printStackTrace(); } return new
	 * ResponseModel(bestNames, bestURLS, IMDBDetials); }
	 */

	// ///////////////////////////// [STOP] FOR MULTIPART FORM DATA... IF
	// REQUIRED ////////////////////////////////
	...
	public @ResponseBody ResponseModel uploadObject4(
			@RequestBody FlickwizImage fi) {
		...
		try {
			byte[] imageBytes = javax.xml.bind.DatatypeConverter
					.parseBase64Binary(fi.getBase64Code());
			...
			for (int i = 0; i < 5; i++) {
				...
				IMDBDetials
						.add(i, getImdbData(similarIndices.get(i).getName()));
			}
	...
	// /////////////// Private Feature Calculation Method /////////////////
	...
	private void allFeaturesExtraction() throws IOException {
		...
		reader.close();
		// return
		// "Features extracted from all posters and stored successfully:"+posters_TrainDescriptors.size();
	}
	 ...
	 * int counter = 0; Loader.init(); featuresORB = new FeaturesORB(); String[]
	 * nextLine; // String checkString = new String(); CSVReader reader = new
	 * CSVReader(new FileReader("movieFile/movies.csv"), ',', '\'', 1); while
	 * ((nextLine = reader.readNext()) != null) { // nextLine[] is an array of
	 * values from the line String imageUrl =
	 * (String.valueOf(nextLine[1].charAt(0)).equals( "\"") ?
	 * nextLine[1].substring(1, nextLine[1].length() - 1) : nextLine[1]);
	 * 
	 * String imageName = (String.valueOf(nextLine[0].charAt(0)).equals( "\"") ?
	 * nextLine[0].substring(1, nextLine[0].length() - 1) : nextLine[0]);
	 * 
	 * 
	 * 
	 * posters_TrainDescriptors.add(counter, featuresORB
	 * .getORBFeaturesDescriptorMat(Converter.img2Mat(ImageIO .read(new
	 * URL(imageUrl)))));
	 * 
	 * posterNames.add(counter, imageName); posterUrls.add(counter, new
	 * URL(imageUrl));
	 * 
	 * ++counter;
	 * 
	 * } reader.close(); return
	 * "Features extracted from all posters and stored successfully:"
	 * +posters_TrainDescriptors.size(); }
	 */
